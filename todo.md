...
## Phase 12D: resupify.com Canonical API Routing + Stripe Webhook
- [x] Audit hosting layout: confirmed frontend + backend co-located (same Express server, production static serve)
- [x] Confirmed frontend uses same-origin relative /api/* paths (url: "/api/trpc" in main.tsx, no hardcoded domains)
- [x] Confirmed /api/stripe/webhook uses express.raw() before express.json() (registerStripeWebhook called first in index.ts)
- [x] Confirmed STRIPE_WEBHOOK_SECRET_V2 is set and used for signature verification
- [x] Stripe Dashboard Test Mode webhook URL: https://resupify.com/api/stripe/webhook (requires manual Stripe Dashboard update — see delivery)
- [x] Webhook endpoint on resupify.com returns 400 for unsigned requests (correct — route is live, not 404)
- [x] Regression: resupify.com loads (HTTP 200), /api/trpc/auth.me returns 200 OK
- [x] Full test suite: 1901 tests passing, 0 TypeScript errors
